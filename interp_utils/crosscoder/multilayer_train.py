...
from .multilayer_model import MultiLayerSPARCCrossCoder, create_multilayer_crosscoder
from .train import QualityGateError, check_quality_gate
from .utils import flush_gpu, get_checkpoint_dir, get_device, get_metrics_dir, save_json, set_seed
...
def _mean_lists(items: list[list[float]]) -> list[float]:
    if not items:
        return []
    n = len(items[0])
    return [sum(row[i] for row in items) / len(items) for i in range(n)]


def _multilayer_model_config(crosscoder: MultiLayerSPARCCrossCoder, layers: list[int]) -> Dict:
    return {
        "crosscoder_kind": "multilayer_sparc",
        "input_dim": int(crosscoder.input_dim),
        "n_layers": int(crosscoder.n_layers),
        "layers": [int(layer) for layer in layers],
        "expansion_factor": int(crosscoder.expansion_factor),
        "dict_size": int(crosscoder.dict_size),
        "topk": int(crosscoder.topk),
        "topk_mode": crosscoder.topk_mode,
        "forced_shared_fraction": float(crosscoder.forced_shared_fraction),
    }


def save_multilayer_checkpoint(
    model: MultiLayerSPARCCrossCoder,
    optimizer,
    epoch: int,
    metrics: dict,
    checkpoint_dir: Path,
    ...
    checkpoint = {
        ...
        "metrics": metrics,
        "model_config": _multilayer_model_config(model, layers),
    }
...
def train_multilayer_crosscoder(
    ...
    if results_dir is None:
        raise ValueError("results_dir is required for multi-layer training")
    ...
    crosscoder = create_multilayer_crosscoder(
        input_dim=input_dim,
        n_layers=n_layers,
    ...
    label = f"{base_model_id}/{aligned_run_id}/layers={layers}/{position}"
    print(f"\nTraining multi-layer cross-coder for {label}")
    print(f"  Dict size: {crosscoder.dict_size}, TopK: {crosscoder.topk}, TopK mode: {topk_mode}")
    ...
    for epoch in epoch_pbar:
        ...
        train_l0_base_by_layer = []
        train_l0_aligned_by_layer = []
        train_self_recon = []
        train_cross_recon = []
        train_sparsity = []
        ...
        for batch in batch_pbar:
            ...
            train_l0_base_by_layer.append(l0["l0_base_by_layer"])
            train_l0_aligned_by_layer.append(l0["l0_aligned_by_layer"])
        ...
        avg_val_fve_aligned = sum(val_fve_aligned_list) / len(val_fve_aligned_list) if val_fve_aligned_list else 0
        ...
        if (epoch + 1) % checkpoint_every == 0:
            ...
            save_multilayer_checkpoint(crosscoder, optimizer, epoch + 1, metrics, checkpoint_dir, layers)
    ...
    save_multilayer_checkpoint(crosscoder, optimizer, num_epochs, final_metrics, checkpoint_dir, layers, is_final=True)
...
def load_trained_multilayer_crosscoder(
    results_dir: Path,
    ...
    if model_config:
        checkpoint_topk_mode = str(model_config["topk_mode"])
        if topk_mode is not None and checkpoint_topk_mode != topk_mode:
            raise ValueError(
                f"Checkpoint topk_mode {checkpoint_topk_mode!r} does not match requested topk_mode {topk_mode!r}"
            )
        checkpoint_layers = [int(layer) for layer in model_config.get("layers", [])]
        if layers is not None and checkpoint_layers and checkpoint_layers != [int(layer) for layer in layers]:
            raise ValueError(f"Checkpoint layers {checkpoint_layers} do not match requested layers {layers}")
        crosscoder = create_multilayer_crosscoder(
            input_dim=int(model_config["input_dim"]),
            n_layers=int(model_config["n_layers"]),
            expansion_factor=int(model_config["expansion_factor"]),
            topk=int(model_config["topk"]),
            topk_mode=checkpoint_topk_mode,
            forced_shared_fraction=float(model_config["forced_shared_fraction"]),
        ).to(device)
    else:
        if input_dim is None or n_layers is None:
            raise ValueError(
                f"Checkpoint {checkpoint_path} has no model_config; input_dim and n_layers are required for legacy load"
            )
        crosscoder = create_multilayer_crosscoder(
            input_dim=input_dim,
            n_layers=n_layers,
            topk_mode=topk_mode or config.MULTILAYER_TOPK_MODE,
        ).to(device)
...
def compute_all_multilayer_feature_activations(crosscoder: MultiLayerSPARCCrossCoder, activations_data: Dict) -> Dict:
    device = get_device()
    crosscoder.eval()
