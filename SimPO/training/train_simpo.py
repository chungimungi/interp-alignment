...
from .config import (
    ...
    BETA, SIMPO_GAMMA, CPO_ALPHA, MAX_LENGTH,
...
def train_and_push(
    ...
    from trl import CPOConfig, CPOTrainer  # SimPO uses CPOTrainer

    from training.dataset import normalize_example
    ...
    dataset = dataset.map(normalize_example, fn_kwargs={"tokenizer": tokenizer})
    ...
    # ── 5. SimPO config ───────────────────────────────────────────────────────
    # SimPO (arXiv:2405.14734) is accessed via CPOTrainer with loss_type="simpo".
    # Key differences from DPO:
    #   - loss_type="simpo"    → length-normalized log prob reward
    #   - cpo_alpha=0.0        → no behavior cloning regularization
    #   - simpo_gamma          → target reward margin
    #   - No reference model loaded (saves memory)
    ...
    # CPOTrainer with loss_type="simpo" — no reference model needed
    trainer = CPOTrainer(
        model=model,
        args=cpo_config,
...
def main(
    ...
    if not dry_run and not light_run:
        ...
        volume_name = "simpo-model-outputs"
        for remote_path, label in [
            ("SmolLM3-3B-SimPO", "adapter"),
            ("SmolLM3-3B-SimPO-merged", "merged"),
        ]:
